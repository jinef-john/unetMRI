    ...
    for cycle in range(CYCLES):
        ...
        for epoch in range(EPOCHS_PER_CYCLE):
            ...
            for batch in loader:
                ...
                print(f"[DIAG] Batch size B: {imgs.shape[0]}")
                ...
                with autocast(device_type='cuda'):
                    # === Watermark generation (mixed precision, memory saving) ===
                    ...
                    # === Normalize for classifier, FORCE FP32 OUTSIDE autocast ===
                    norm = lambda x: (x - 0.5) / 0.5
                    # --- Важно! После autocast выход может быть HalfTensor (FP16), но C1/C2 нужны только FP32 ---
                ...
                sparsity_weight = 0.15  # termporary, very large 0.03, but make 0.15 to break up continous heatmap
                ...
                #    (wm_img.clamp(0, 1) * 2 - 1), (clean_img.clamp(0, 1) * 2 - 1)
                #).mean()
                alpha_l1 = 0.075
                #alpha_lpips = 0.075
            ...
            for i in range(B):
                ...
                try:
                    # 1. Original
                    # 1. Original (grayscale to pseudo-RGB for visualization)
                    orig_img = imgs[i].detach().cpu().numpy()[0]  # [H, W], извлекаем единственный канал
                    orig_img = (orig_img * 255).clip(0, 255).astype(np.uint8)  # [H, W], uint8
                    orig_img_rgb = np.stack([orig_img] * 3, axis=-1)  # [H, W, 3] для cv2/PIL совместимости
                    ...
                    # 2. Watermarked (grayscale to pseudo-RGB for visualization)
                    wm_img_np = wm_img[i].detach().cpu().numpy()[0]  # [H, W]
                    wm_img_np = (wm_img_np * 255).clip(0, 255).astype(np.uint8)
                    wm_img_rgb = np.stack([wm_img_np] * 3, axis=-1)  # [H, W, 3]
                    wm_img_rgb = cv2.resize(wm_img_rgb, (512, 512), interpolation=cv2.INTER_CUBIC)
                    wm_img_labeled = draw_label(wm_img_rgb, "Watermarked")

                    # --- Priority mask overlays (non-overlapping effective masks)
                    # Up to 0.5 threshold for "allowed"
                    ...
                    # Allowed zones: union of all effective masks
                    allowed_mask = ((mask_latent_eff > 0.5) | (mask_skip64_eff > 0.5) | (mask_skip128_eff > 0.5))[0]
                    allowed_mask_3c = np.repeat(allowed_mask[:, :, np.newaxis], 3, axis=2)
                    # Overlay: белый в разрешённых зонах, оригинал в остальных
                    ...
                    # 4-6. Individual mask overlays (цветные)
                    ...
                    # 7-9. Heatmaps: энергия watermark'а на каждом уровне
                    ...
                    cname = dataset.classes[ys[i].item()] if hasattr(dataset, 'classes') else CLASSES[ys[i].item()]
