...
CLEAN_HIGH   = 0.35   # если выше — увеличиваем payload (пусть путается)
...
def make_collage_save(save_path, raw_gray_512, clean_512, wm_512,
    ...
    images = [
        ...
        make_heat((diff_512 - diff_512.min()) / (diff_512.max() - diff_512.min() + 1e-8)), # 3. Diff Heatmap
        cv2.addWeighted(gray3(clean_512), 0.7, (np.stack([u2_512*255]*3, axis=-1)).astype(np.uint8), 0.3, 0) # 4. U2Net Overlay
    ]
    row1_labels = ['Original', 'Clean', 'Watermarked', 'Diff', 'U2Net Overlay']

    maskability_hm = make_heat(np.clip(maskability_512,0,1))
    allow_vis      = (np.stack([allow_bin_512*255]*3, axis=-1)).astype(np.uint8)
    allow_vis[:,:,2] = allow_vis[:,:,2]*0 + 255  # BGR: mark in red-ish
    allow_vis = np.clip(allow_vis,0,255)
    u2_vis   = (np.stack([u2_512*255]*3, axis=-1)).astype(np.uint8)

    row2_images = [
        cv2.addWeighted(gray3(clean_512), 0.7, maskability_hm, 0.3, 0),    # 0. Maskability Overlay
        cv2.addWeighted(gray3(clean_512), 0.7, allow_vis, 0.3, 0),         # 1. Allow Overlay
        maskability_hm,                                                    # 2. Maskability
        allow_vis,                                                         # 3. Allow Mask
        u2_vis                                                             # 4. U2Net Mask
    ]
    row2_labels = ['Maskability Ovrl', 'Allow Ovrl', 'Maskability', 'Allow Mask', 'U2Net Mask']
    ...
    for i, (img, label) in enumerate(zip(row2_images, row2_labels)):
        cv2.putText(img, label, (10, 36), font, font_scale, font_color, thickness, cv2.LINE_AA)
        row2_images[i] = img
...
def main():
    ...
    gen_optim = torch.optim.AdamW(watermark_gen_cnn.parameters(), lr=1e-3)
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        for it, (img_1, ys, fnames, cls_names) in enumerate(loader):
            ...
            # ---- Генерация watermark через CNN ----
            wm_lat, wm_skip = watermark_gen_cnn(img_1, allow_lat, allow_skip, intensity)
            lat_wm = latents + wm_lat
            skip_wm = skip64s + wm_skip
            wm_img = run_decoder(decoder, lat_wm, skip_wm)  # B×1×512×512, в [0,1]
            ...
            with torch.autocast(device_type='cuda', enabled=AMP):
                ...
                adv_c2_loss = ALPHA_CLEAN_ENTROPY * loss_clean_c2 + 1.5 * loss_wm_c2
                ...
                EDGE_WEIGHT = 0.05
                total_loss = adv_c2_loss + L1_WEIGHT * l1_img + SSIM_WEIGHT * ssim_loss + EDGE_WEIGHT * edge_loss
            ...
            for b in range(min(2, img_1.size(0))):
                ...
                make_collage_save(save_path, raw_np, cln_np, wm_np1, u2_np, allow_np, maskab_np, diff_np)
