...
ENCODER_PTH = r"E:\MRI_LOWMEM\Encoder_latent_64_128\autoencoder_epoch9.pth"
...
# ============ TRAINING LOOP ============
def main():
    dataset = MRIImageDataset(DATA_ROOT, CLASSES)
    ...
    for cycle in range(CYCLES):
        ...
        for epoch in range(EPOCHS_PER_CYCLE):
            ...
            epoch_metrics = []
            for batch in loader:
                imgs, ys, fnames = batch
                imgs = imgs.to(device, non_blocking=True)
                ys = ys.to(device, non_blocking=True)
                B = imgs.shape[0]
                # LIVE-энкодинг латентов
                with torch.no_grad():
                    latents, skip64s, skip128s = encoder(imgs)
                ...
                with autocast(device_type="cuda"):
                    ...
                    # skip128 без watermark (или внедрять аналогично, если нужно)
                    wm_img = decoder(watermarked_latent, watermarked_skip64, skip128s)
